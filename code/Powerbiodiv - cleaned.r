...
optionCurate <- FALSE
if (optionCurate) {
  myFile <- "PowerBiodiv_systematic_review_data_v20241017_BL.xlsx"
  ...
  myDataNum <- myDataClean[ , unlist(allSelections)]
  ...
  colnames(myDataNumImp) <- colnames(myDataFacImp) <- unlist(allSelections)
  for (i in names(allSelections) ) {  # impute by groups separately
    set.seed(1234)
    myDataFacImp[ , allSelections[[i]] ] <- as.data.frame(lapply(imputeFAMD(myDataFac[, allSelections[[i]]], ncp = 2)$completeObs, factor, ordered = TRUE))
